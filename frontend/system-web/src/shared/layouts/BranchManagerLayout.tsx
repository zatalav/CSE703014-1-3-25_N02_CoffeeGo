...
import {
  LayoutDashboard, ShoppingCart, Users, PackageSearch,
  BarChart2, Settings, Bell, Search, LogOut, KeyRound,
  ChevronDown, ChevronRight, Menu, Store, Clock,
  AlertTriangle, X, User,
} from "lucide-react";
import { api } from "../../lib/api";
import { useAuth } from "../../lib/auth";
import type { PageResponse } from "../../lib/types";
import { loadInventoryData, pageItems, type InventoryData } from "../../features/inventory-data";

export interface BMNavItem {
  id: string;
  label: string;
  icon: React.ComponentType<{ size?: number; className?: string; style?: React.CSSProperties }>;
}

const navItems: BMNavItem[] = [
  { id: "bm-dashboard", label: "Tổng quan", icon: LayoutDashboard },
  { id: "bm-orders", label: "Đơn hàng", icon: ShoppingCart },
  { id: "bm-staff", label: "Nhân viên & Lịch", icon: Users },
  { id: "bm-inventory", label: "Kho nguyên liệu", icon: PackageSearch },
  { id: "bm-reports", label: "Báo cáo", icon: BarChart2 },
  { id: "bm-settings", label: "Cài đặt chi nhánh", icon: Settings },
];
...
export function BranchManagerLayout({
  ...
  breadcrumb,
  onSwitchToAdmin,
}: BranchManagerLayoutProps) {
  const [mobileOpen, setMobileOpen] = useState(false);
  const [showUserMenu, setShowUserMenu] = useState(false);
  const [showAlert, setShowAlert] = useState(true);
  const [inventoryData, setInventoryData] = useState<InventoryData | null>(null);
  const [orders, setOrders] = useState<BranchOrderSummary[]>([]);
  ...
  const loadInventorySummary = async () => {
    try {
      ...
      const [inventory, orderPayload] = await Promise.all([
        loadInventoryData(branchId),
        api.get<PageResponse<BranchOrderSummary> | BranchOrderSummary[]>(`/orders?size=200${branchQuery}`).catch(() => []),
      ]);
      setInventoryData(inventory);
      setOrders(pageItems(orderPayload).filter(order => !branchId || order.branchId === branchId));
    } catch {
  ...
  const lowStockItems = useMemo(() => {
    ...
    return ingredients.filter(item => item.minLevel > 0 && item.current <= item.minLevel);
  }, [inventoryData?.ingredients]);

  const lowStockText = useMemo(() => {
    const names = lowStockItems.slice(0, 3).map(item => item.name).join(", ");
    if (!names) return "";
    return `${names}${lowStockItems.length > 3 ? ` và ${lowStockItems.length - 3} mặt hàng khác` : ""}`;
  }, [lowStockItems]);

  const branchName = inventoryData?.branches.find(branch => branch.id === branchId)?.name
    || (branchId ? `Chi nhánh #${branchId}` : "Chi nhánh");
  const todayOrders = useMemo(() => orders.filter(order => isToday(order.createdAt)), [orders]);
  const pendingOrderCount = orders.filter(order => ["pending", "confirmed", "preparing"].includes((order.status || "").toLowerCase())).length;
  const todayRevenue = todayOrders.reduce((sum, order) => sum + Number(order.total ?? order.amount ?? 0), 0);
  const todayCustomers = new Set(todayOrders.map(order => order.customerId).filter(Boolean)).size;

  const SidebarContent = () => (
    <div
      className="flex flex-col h-full"
      style={{ backgroundColor: "#1a5276", width: 240 }}
    >
      {/* Branch identity */}
      <div className="px-4 py-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          >
            <Store size={20} style={{ color: "#1a5276" }} />
          </div>
          <div className="min-w-0">
            <p className="text-white font-semibold text-sm leading-tight truncate">{branchName}</p>
            <div className="flex items-center gap-1.5 mt-1">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
              <span className="text-xs" style={{ color: "rgba(255,255,255,0.6)" }}>Đang hoạt động</span>
            </div>
          </div>
        </div>
        ...
        >
          <div
            ...
            style={{ backgroundColor: "#F59E0B", color: "#1a5276" }}
          >
            {initials}
          </div>
          <div className="min-w-0">
      ...
      <div className="flex-1 overflow-y-auto py-3 px-2" style={{ scrollbarWidth: "none" }}>
        ...
        {navItems.map(item => {
          ...
          return (
            ...
            >
              <Icon
                size={18}
                style={{ color: active ? "#F59E0B" : "rgba(255,255,255,0.6)", flexShrink: 0 }}
              />
              <span
                className="text-sm"
                style={{
              ...
              {item.id === "bm-orders" && pendingOrderCount > 0 && (
                <span
                  className="ml-auto text-xs font-bold px-1.5 py-0.5 rounded-full"
                  style={{ backgroundColor: "#EF4444", color: "#fff", fontSize: 10 }}
                >
                  {Math.min(pendingOrderCount, 99)}
                </span>
              )}
            </button>
          );
        ...
        <div className="mt-4 mx-1">
          ...
          >
            {[
              { label: "Doanh thu", value: `${formatShortMoney(todayRevenue)} đ`, color: "#F59E0B" },
              { label: "Đơn hàng", value: `${todayOrders.length} đơn`, color: "#10B981" },
              { label: "Khách hàng", value: `${todayCustomers} KH`, color: "#60A5FA" },
            ].map((s, i) => (
      ...
      <div className="flex-shrink-0 px-4 py-3 border-t border-white/10">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs" style={{ color: "rgba(255,255,255,0.5)" }}>
            {currentTime.toLocaleString("vi-VN", { weekday: "long", hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit", year: "numeric" })}
          </span>
        </div>
      </div>
    </div>
    ...
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: "#F0F4F8" }}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {showAlert && lowStockItems.length > 0 && (
          <div
            className="flex-shrink-0 flex items-center justify-between px-4 py-2.5 text-sm"
            style={{ backgroundColor: "#FFF9E6", borderBottom: "1px solid #FDE68A" }}
          >
            <div className="flex items-center gap-2">
              <AlertTriangle size={14} style={{ color: "#F59E0B" }} />
              <span style={{ color: "#92400E" }}>
                <strong>Cảnh báo:</strong> {lowStockText} sắp hết tồn kho - cần đặt hàng ngay
              </span>
            </div>
            <button onClick={() => setShowAlert(false)} className="p-1 text-amber-600 hover:text-amber-800">
              <X size={14} />
            </button>
          </div>
        )}
        ...
        {/* Topbar */}
        <header
          className="flex-shrink-0 h-14 bg-white border-b border-gray-200 flex items-center px-4 gap-3"
        ...
        >
          ...
          <div
            ...
            style={{ backgroundColor: "#EBF5EB", color: "#065F46" }}
          >
          ...
          <div className="relative hidden sm:block">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 text-sm bg-gray-100 border border-transparent rounded-lg focus:outline-none focus:bg-white focus:border-gray-300"
              style={{ width: 160 }}
            />
          </div>
          ...
          >
            ...
            {lowStockItems.length > 0 && (
              ...
              >
                <span className="text-white font-bold" style={{ fontSize: 9 }}>{Math.min(lowStockItems.length, 9)}</span>
              </span>
            )}
          </button>
          ...
          <div className="relative">
            ...
            >
              <div
                ...
                style={{ backgroundColor: "#1a5276", color: "#F59E0B" }}
              >
                {initials}
              </div>
            ...
            {showUserMenu && (
              <>
                ...
                <div
                  ...
                  style={{ width: 180, boxShadow: "0 8px 24px rgba(0,0,0,0.12)" }}
                >
                  ...
                  <button
                    onClick={() => {
                      ...
                      onNavigate("bm-profile");
                    }}
                    className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-600 text-left transition-colors"
                  ...
                  <button className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-600 text-left transition-colors"
                    onMouseEnter={e => (e.currentTarget as HTMLElement).style.backgroundColor = "#F9FAFB"}
                    onMouseLeave={e => (e.currentTarget as HTMLElement).style.backgroundColor = "transparent"}
                  >
                  ...
                  <button onClick={() => void logout()} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-600 text-left transition-colors"
